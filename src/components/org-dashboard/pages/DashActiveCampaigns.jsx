      ...
      campaigns.filter((campaign) => {
        return (
          ...
          (campaign.targetAmount &&
            campaign.targetAmount.toString().includes(searchTerm)) ||
          (campaign.description &&
            campaign.description.toString().includes(searchTerm))
    ...
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .delete(`${apiUrl}/api/v1.0/deletecampaign/${campaignId}`, config)
            .then((res) => {
              // console.log(res)
              if (res.status === 200) {
                Swal.fire({
                ...
                Swal.fire("Error!", "The campaign did not deactivate", "error");
              }
    ...
    <div className="h-screen lg:h-fit">
      <div className="text-sm breadcrumbs ml-2">
        <ul>
          ...
          <li>
            <a href="/org/dashboard/mycampaigns/active">Active Fundraisers</a>
          </li>
        </ul>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 my-3">
        <div className="flex justify-start gap-3">
          ...
          <div>
            <a href="/org/dashboard/mycampaigns/inactive">
              <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline mb-4">
      ...
      {allCampaigns.length > 0 && paginatedCampaigns.length > 0 ? (
        <>
            ...
            <table className="table table-xs bg-white">
              ...
              <thead className="text-gray-800 bg-gray-100 text-left">
                <tr>
                  ...
                  <th className="px-2 py-2 font-medium leading-4 tracking-wider text-leftuppercase border-b border-gray-200">
                    Campaign Name
                  </th>
                  <th className="px-2 py-2 font-medium leading-4 tracking-wider text-leftuppercase border-b border-gray-200">
                    Category
                  ...
                  paginatedCampaigns.map((item) => {
                    return (
                      <tr key={item.id} className="even:bg-blue-50">
                        ...
                        <td className="px-2 py-2 whitespace-no-wrap border-b border-gray-200">
                          {walletDetails &&
                            walletDetails[item.id]?.available_balance}
                        </td>
                        <td className="px-2 py-2 whitespace-no-wrap border-b border-gray-200">
                          <button
                            onClick={() => {
                              ...
                              setCampaign(item);
                              setCurrentlWebUrl(
                                `https://www.msaadafund.com/campaigns/${formatSlug(
                                  item.campaignName
                                )}`
                              );
                            }}
                            className="bg-green-600 hover:bg-green-700 text-lg text-white font-bold py-2 px-3 rounded focus:outline-none focus:shadow-outline"
                          >
                            <IoShareSocialSharp />
                          </button>
                        </td>
                        <td className="px-2 py-2 whitespace-no-wrap border-b border-gray-200">
                          <button
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-lg text-white font-bold py-2 px-3 rounded focus:outline-none focus:shadow-outline ml-2 w-full sm:w-auto"
                          >
                            <FaEdit />
                          </button>
                        </td>
                        <td className="px-2 py-2 whitespace-no-wrap border-b border-gray-200">
                          <button
                            ...
                            className="bg-red-600 hover:bg-red-700 text-lg text-white font-bold py-2 px-3 rounded focus:outline-none focus:shadow-outline"
                          >
          ...
          <div className="flex justify-center my-4 join grid-cols-2">
            ...
            <button
              className="btn btn-outline h-5 join-item btn-sm"
        ...
        <div className="grid grid-cols-1 gap-4 mt-3 px-1">
          <div>
            <p className="text-red-500">
              No active campaigns to display. Create campaign to start receiving
              contributions
            </p>
          </div>
        </div>
              ...
              <div>
                ...
                <div className="mt-0 flex justify-evenly space-x-3">
                  ...
                  <TelegramShareButton
                    url={currentlWebUrl}
                    title={`${
                      campaign && campaign.organisation && campaign.campaignName
                    }`}
                  >
