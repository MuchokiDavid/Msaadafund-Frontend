...
function DashInactiveCampaigns({ allCampaigns, campaignError }) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${apiUrl}/api/v1.0/get_inactive`, config)
      .then((res) => {
  ...
  useEffect(() => {
    const filtered =
      inActiveCampaign &&
      inActiveCampaign.filter((campaign) => {
        return (
    ...
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .patch(
              `${apiUrl}/api/v1.0/activate/campaign/${campaignId}`,
              {},
              config
            )
            .then((res) => {
              ...
              if (res.status === 200) {
                ...
                Swal.fire("Error!", "The campaign did not activate", "error");
              }
            })
            .catch((err) => {
              const errorMsg = err.response?.data?.error || "An error occurred";
  ...
  return (
    <div className="h-screen lg:h-fit">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 my-3">
        <div>
          <a href="/org/dashboard/mycampaigns/active">
                  ...
                  paginatedCampaigns.map((item) => {
                    return (
                      <tr key={item.id} className="even:bg-blue-50">
                        ...
                        <td className="px-4 py-1 whitespace-no-wrap border-b border-gray-200 ">
                          <button
                            onClick={() => activateCampaign(item.id)}
        ...
        <div className="grid grid-cols-1 gap-4 mt-3 px-1">
          <div>
            <p className="text-red-500">
              No Inactive campaigns to display at the moment.
            </p>
          </div>
          <div>
